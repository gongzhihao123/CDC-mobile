...
export default {
  ...
  methods: {
      ...
      apiGetAntifatInfo(this.activityId, studentId)
        .then(res => {
          if (res.code === 1) {
            ...
            this.$set(this.fatInfo, 'beforeBmi', res.data.antifat.beforeBmi)
            this.$set(this.fatInfo, 'afterBmi', res.data.antifat.afterBmi)
          }
        })
    },
    /**
    ...
    // 跳转记录页
    navClockInGoRecord () {
      this.$router.push({ path: '/record', query: { activityId: this.activityId, activityType: this.activityType } })
    },
    /**
     * 跳转分享
     */
